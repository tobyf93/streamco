...
describe('POST /', function() {
  ...
  describe('when valid JSON is received', function() {
    ...
    describe('and in the correct format', function() {
      it("should return 200 response with data", function(done) {
        request.post("http://localhost:3000", correctJSON, function(err, res, body){
          expect(res.statusCode).toBe(200);
          expect(errorReturned(body)).toBe(false);
          done();
        });
      });
    });
...
var correctJSON = JSON.stringify({
    "payload": [
        {
            "country": "UK",
